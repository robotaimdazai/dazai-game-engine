...
public:
    ...
    {
        assert(m_entity_to_index_.find(entity.id) == m_entity_to_index_.end() && "Component added to same entity more than once.");
        const size_t new_index = m_size_;
        m_entity_to_index_[entity.id] = new_index;
        m_index_to_entity_[new_index] = entity.id;
    ...
    {
        assert(m_entity_to_index_.find(entity.id) != m_entity_to_index_.end() && "Removing non-existent component.");
        //move last entity to the removed entity's location
        const size_t index_of_removed_entity = m_entity_to_index_[entity.id];
        ...
        m_entity_to_index_.erase(entity.id);
        m_index_to_entity_.erase(index_of_last_element);
        --m_size_;
    }
    auto get_data(const entity entity) ->T&
    {
        assert (m_entity_to_index_.find(entity.id) != m_entity_to_index_.end() && "Component does not exist.");
        return m_component_array_[m_entity_to_index_[entity.id]];
    }
    auto entity_destroyed(const entity entity) ->void override
    {
        if(m_entity_to_index_.find(entity.id) != m_entity_to_index_.end())
        {
